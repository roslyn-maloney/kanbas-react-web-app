import { MdDoNotDisturbAlt } from "react-icons/md";
import { FaCheckCircle } from "react-icons/fa";
import { BiImport } from "react-icons/bi";
import { LiaFileImportSolid } from "react-icons/lia";
import { IoMdHome } from "react-icons/io";
import { GiNetworkBars } from "react-icons/gi";
import { FaBullhorn } from "react-icons/fa";
import { CiBellOn } from "react-icons/ci";
import "./Style.css";


{/* Find more icons */ }
export default function CourseStatus() {
  return (
    <div id="wd-course-status" style={{ width: "300px"}}>
      <div style={{ marginLeft: '20px' }}>
        <h2>Course Status </h2>
      </div>
      <div className="d-flex">
        <div className="w-50 pe-1">
          <button className="button-home btn btn-lg btn-secondary w-100 text-nowrap ">
            <MdDoNotDisturbAlt className="me-2 fs-5" /> Unpublish </button>
        </div>
        <div className="w-50">
          <button className="button-home btn btn-lg btn-success w-100">
            <FaCheckCircle className="me-2 fs-5" /> Publish </button>
        </div>
      </div><br />
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <BiImport className="me-2 fs-5" /> Import Existing Content </button>
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <LiaFileImportSolid className="me-2 fs-5" /> Import from Commons </button>
      {/* Choose Home page */}
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <IoMdHome className="me-2 fs-5" /> Choose Home Page </button>
      {/* Veiw Corse Screen */}
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <GiNetworkBars className="me-2 fs-5" /> View Course Screen </button>
      {/* New Announcement */}
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <FaBullhorn className="me-2 fs-5" /> New Announcement </button>
      {/* New Analytics */}
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <GiNetworkBars className="me-2 fs-5" /> New Analytics </button>
      {/* View Course Notifications */}
      <button className="button-home btn btn-lg btn-secondary w-100 mt-1 text-start">
        <CiBellOn className="me-2 fs-5" /> View Course Notifications </button>
    </div>
  );
}